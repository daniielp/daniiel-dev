...
async function createContact(req: ContactApiRequest, res: NextApiResponse) {
  if (req.method === "POST") {
    ...
    if (body == null) {
      return res
        .status(400)
        .json({ code: "bad_request", message: "Missing body" });
    }
    ...
    if (
      ...
      body.name == null ||
      body.subject == null ||
      body.recaptchaResponse == null
    ) {
      return res
        .status(400)
        .json({ code: "bad_request", message: "Field(s) missing" });
    }
